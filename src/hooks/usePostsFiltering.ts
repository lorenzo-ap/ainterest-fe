...
import { type FiltersState, type PostModel, SortCriteria, SortOrder } from '../types';
...
export const usePostsFiltering = (posts: PostModel[], options: UsePostsFilteringOptions = {}) => {
	...
	const handleFiltersChange = (criteria: SortCriteria) => {
		let order: SortOrder;
		if (filters.criteria === criteria) {
			order = filters.order === SortOrder.ASCENDING ? SortOrder.DESCENDING : SortOrder.ASCENDING;
		} else {
			order = SortOrder.ASCENDING;
		}
